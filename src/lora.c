...
static void snake_screen_blanker (int init)
{
   ...
   if (init == 1) {
      ...
      elemx[2][0] = x[2] = 60;
      elemy[2][0] = y[2] = 12;
      prints (y[0] - 1, x[0] - 1, LGREEN|_BLACK, "ÛÛ");
      prints (y[1] - 1, x[1] - 1, LRED|_BLACK, "ÛÛ");
      prints (y[2] - 1, x[2] - 1, BLUE|_BLACK, "ÛÛ");
   }
   // Eliminazione dello screen blanker
   else if (init == 2) {
      return;
   ...
   // fatto.
      ...
      for (m = 0; m < 3; m++) {
         ...
         for (i = 0; i < MAX_ELEMENTS; i++) {
            ...
            if (i == 0) {
               if (m == 0)
                  prints (elemy[m][i] - 1, elemx[m][i] - 1, LGREEN|_BLACK, "ÛÛ");
               else if (m == 1)
                  prints (elemy[m][i] - 1, elemx[m][i] - 1, LRED|_BLACK, "ÛÛ");
               else
                  prints (elemy[m][i] - 1, elemx[m][i] - 1, BLUE|_BLACK, "ÛÛ");
            }
