...
int global;
{
   ...
   if(!sys.freearea && !usr.xfer_prior && config->class[usr_class].ratio && !name[0] && usr.n_dnld >= config->class[usr_class].start_ratio) {
...
int flag;
{
      ...
      if (findfirst (filename, &blk, 0)) {
         ...
         if (flag) {
            ...
            while (read(fd, (char *)&tsys.file_name, SIZEOF_FILEAREA) == SIZEOF_FILEAREA) {
               ...
               do {
                  if ((((fl + blk.ff_fsize) / 1024) + usr.dnldl) > config->class[usr_class].max_dl && !sys.freearea && !usr.xfer_prior) {
   ...
   if(((fl/1024) + usr.dnldl) > config->class[usr_class].max_dl && !sys.freearea && !usr.xfer_prior) {
