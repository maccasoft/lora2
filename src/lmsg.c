...
{
   int fdu, fd, last, i, msgnum, kill, nummsg, maxmsgs;
   ...
   while ( read(fd, (char *)&tsys.msg_name, SIZEOF_MSGAREA) == SIZEOF_MSGAREA ) {
      ...
      printf ("%4d / %-4d\b\b\b\b\b\b\b\b\b\b\b", nummsg - 1, maxmsgs);
...
{
   int fds, fd, first, last, i, m, msgnum, prev, next, lwrite;
   ...
   while ( read (fds, (char *)&tsys.msg_name, SIZEOF_MSGAREA) == SIZEOF_MSGAREA ) {
      ...
      clreol ();
      printf("   юд %3d - %-25.25s %-25.25s %d Msg.\r", tsys.msg_num, tsys.msg_name, tsys.echomail ? tsys.echotag : "", msgnum);
...
void Purge_Quick_Areas (int renum)
{
   ...
   while (read(fd, (char *)&msghdr, sizeof (struct _msghdr)) == sizeof (struct _msghdr)) {
      flag = 0;
      if (msghdr.board >= 1 && msghdr.board <= 200 && msghdr.msgnum) {
         pnum[msghdr.board-1]++;
...
void Pack_Quick_Base (int renum)
{
   ...
   while (read_block (fd, buff) == 256) {
      ...
      if ((count % 12) == 0)
         printf("   юд Block: %u / %u, Written: %u\r", count, numblocks, (unsigned int)(wpos / 256L));
   }
